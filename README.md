...
**AI consent (v1.4.8+, Ask/onboarding included):** `POST /api/mobile/v1/scan` enforces the same server-side AI-consent gate (`requireMobileAIConsent`, 403 `ai_consent_required`) as `generate`/`import`/`plan-week`. Mobile Ask/Copilot connections also require `aiConsentAt` (403 `ai_consent_required`). Client-side, consent state (`aiConsentAt`) lives in `SessionStore.hasAIConsent`; a shared `AIConsentCoordinator` gates scan, generate, import, plan week, Ask, and onboarding. The consent gate names **Google Gemini (via Cloudflare AI Gateway)** and **Cloudflare Workers AI**. Users can withdraw consent in Settings → Privacy & AI via `PATCH` with `aiConsentAt: null` (clears the stored timestamp and re-shows the gate).
