...
- **Mobile decode hardening** — Hub `nutritionToday` / `nutritionTrends` coerce D1 string aggregates to JSON numbers and soft-null invalid summaries so Hub still loads. Manifest `personalIntake.occurredAt` is always ISO-8601. iOS tolerantly decodes string numerics / `customFields` and logs DEBUG coding paths on `APIError.decoding`.
