...
- **Resolve** — USDA-shaped self-hosted `NUTRITION_DB` with curated `food_alias` short-circuit, FTS top-N + primary-prefix bank, JS re-rank (matcher `1.4.0`); optional live FDC search miss fallback when `USDA_FDC_API_KEY` is set. Blank on true miss/low for manual/CSV; medium+ USDA attaches in scan review. Labelled **AI Estimate** when `nutrition-ai-estimate` is on and ingest is cargo `scan_review` **or** supply dock-from-scan. Merge into a cargo row with null nutrition backfills USDA/AI from the incoming name. Mobile: `POST /api/mobile/v1/nutrition/resolve`.
