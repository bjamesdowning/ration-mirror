...
- **Resolve** — USDA-shaped self-hosted `NUTRITION_DB` with FTS top-N + primary-prefix bank for fragile heads (e.g. `Milk,%`) + JS re-rank (matcher `1.3.0`, inverted OCR↔USDA labels, peer-deduped margin, modifier fragile blockers); blank on true miss/low for manual/CSV; medium+ USDA attaches in scan review. Labelled **AI Estimate** when `nutrition-ai-estimate` is on and ingest is cargo `scan_review` **or** supply dock-from-scan (client `allowAiEstimate` booleans are ignored; resolve routes keep AI off). Merge into a cargo row with null nutrition backfills USDA/AI from the incoming name. Mobile: `POST /api/mobile/v1/nutrition/resolve`. iOS scan review calls resolve after OCR, shows/edits macros on **Edit item**, and AI intros mention nutrition when `nutrition-engine` is on.
...
- **Dogfood / release** — One API serves App Store `1.3.17`, TestFlight, and web. Flags stay registry-default **off**; enable only via Flagship allowlist **+** `clientPlatform` **+** `clientVersion` ≥ dogfood floor (iOS **`1.3.29+`**, web `APP_VERSION`). After App Review, widen the same version rule so updaters get nutrition without a second backend cut. Never turn nutrition **on** with `FEATURE_FLAG_OVERRIDES`. Full phases: [nutrition-rollout.md](docs/dev/nutrition-rollout.md).
