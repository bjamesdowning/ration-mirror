...
**Public splash experience:** `/` leads with the Ration crate mark + wordmark, the iOS App Store action, and a looping product canvas (scan → pantry → meals → plan → shop → cook → Daily Fuel). A click-to-play YouTube explainer opens in a lightbox. The page then explains the closed kitchen loop: Cargo → Galley → Manifest → Supply → Dock, with a device-frame stage proof and a mobile stage rail. Daily Fuel (`#fuel`) is the private overlay on cook — household stock deducts, personal macros stay private, not medical advice. `#kitchens` explains multiple kitchens (Free: one owned kitchen; Crew: invites and up to five). `#interfaces` still exists as a stable id and presents iOS + Copilot, Web + Copilot, then a compact MCP strip. A header light/dark control changes appearance for this page only (does not write the app theme cookie or hub settings). Motion (hero loop, fuel dials, kitchen switcher, sticky mobile CTA) is paused or reduced by `prefers-reduced-motion`. `#pricing` advertises Crew’s 1 free Ask Ration conversation per group per day and shares the full Free vs Crew feature matrix with `/hub/pricing` via [`PricingFeatureMatrix`](app/components/pricing/PricingFeatureMatrix.tsx); `#pricing` and `#signup` deep links remain stable. Public marketing URLs (`APP_STORE_URL`, help docs, YouTube channel, and explainer video) are centralized in [`app/lib/marketing.ts`](app/lib/marketing.ts).
