...
- **Resolve** — USDA-shaped self-hosted `NUTRITION_DB` with FTS top-N + JS re-rank (matcher `1.2.0`, inverted OCR↔USDA labels, peer-deduped margin); blank on true miss/low for manual/CSV; medium+ USDA attaches in scan review. Labelled AI estimate only when request `ingestSource` is `scan_review` and `nutrition-ai-estimate` is on (client `allowAiEstimate` booleans are ignored; resolve routes keep AI off). Mobile: `POST /api/mobile/v1/nutrition/resolve`. iOS scan review calls resolve after OCR, shows/edits macros on **Edit item**, and AI intros mention nutrition when `nutrition-engine` is on.
