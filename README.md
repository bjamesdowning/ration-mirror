...
- **Replenish (scan-from-Supply)** — The Supply hub **Replenish Cargo** action offers **From receipt** (credit-gated intro modal on web, matching Cargo scan UX → scan/upload → server-side match via `GET /api/supply-lists/:id/scan-match` → review pairings → `POST .../scan-complete` docks to Cargo, runs the same post-dock reconciliation, then reconciles list rows) or **From purchased list** (same as dock cargo above). iOS mirrors the credit intro, consent gate, and camera/PDF source picker before processing.
