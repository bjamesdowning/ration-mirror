...
- **Quick Eat** (Flagship `cargo-quick-eat` + `nutrition-cook-log-split`) — `POST /api/cargo/:id/quick-eat` and `POST /api/mobile/v1/cargo/:id/quick-eat` ensure a linked provision, prepare today’s Manifest **snack**, best-effort pantry deduct (including when stock is 0), and optionally log **private intake for the acting user** (others still see Log serving / +). iOS: yellow trailing **Eat** swipe + detail FAB (outline carrot); Mark Empty remains detail-only.
