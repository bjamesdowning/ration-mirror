...
- **UI** — Cargo/Galley panels (web + iOS when `nutrition-engine` is on), scan review (`POST /api/nutrition/resolve`), Manifest Eat plate-up + preference-driven day/week goal strip (only nutrients you set; **No goals** empty state) + ~13-month calendar, Settings goals (optional nutrients including 0, **explicit** consent; clear returns `{ cleared: boolean }`). Personal kcal stays on user-scoped intake APIs — not in shared Flight Recorder event payloads.
