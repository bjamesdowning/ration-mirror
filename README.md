...
An AI-powered pantry, meal-planning, and macro-tracking application built as a Cloudflare Worker with SSR, AI-powered receipt scanning, semantic ingredient matching, meal generation, weekly meal planning, tiered subscriptions, and multi-tenant group sharing. **Ration Copilot** (Ask Ration) is the first-party assistant on web and iOS; external agents connect via **MCP** (Model Context Protocol) to operate inventory, recipes, meal plans, shopping lists, and (when enabled) personal nutrition from Claude, Cursor, ChatGPT, and other compatible clients. Built for the Cloudflare paid plan — design decisions prioritise correctness, latency, and scale over free-tier frugality.
...
flowchart TB
    ...
    subgraph External["External Services"]
        ...
        Supadata["Supadata — web.scrape + metadata + transcripts"]
    end
...
flowchart TB
    ...
    subgraph AI["Cloudflare AI"]
        AIBinding["Workers AI — Embeddings"]
...
| `AI` | Workers AI | Embedding generation (`@cf/google/embeddinggemma-300m`, 768-dim) on `ration` / `ration-mcp`; Copilot Gemini 3.7 Flash via AI Gateway (`ration-gateway`) on `ration-copilot` |
...
**Public splash experience:** `/` leads with the Ration crate mark + wordmark, the iOS App Store action, and a click-to-play YouTube explainer in a 9:16 phone frame. The page then explains the closed kitchen loop: Cargo → Galley → Manifest → Supply → Dock. Desktop uses a sticky device-frame stage proof; on mobile, each stage is a full-viewport chapter (copy, live kitchen UI, and signal together) with document proximity snap — no nested phone chrome or tab rail. Daily Fuel (`#fuel`) is the private overlay on cook — household stock deducts, personal macros stay private, not medical advice. `#kitchens` explains multiple kitchens (Free: one owned kitchen; Crew: invites and up to five). `#interfaces` still exists as a stable id and presents iOS + Copilot, Web + Copilot, then a compact MCP strip. A header light/dark control changes appearance for this page only (does not write the app theme cookie or hub settings). Motion (fuel dials, kitchen switcher, sticky mobile CTA, loop snap) is paused or reduced by `prefers-reduced-motion`. `#pricing` advertises Crew’s 1 free Ask Ration conversation per group per day and shares the full Free vs Crew feature matrix with `/hub/pricing` via [`PricingFeatureMatrix`](app/components/pricing/PricingFeatureMatrix.tsx); `#pricing` and `#signup` deep links remain stable. Public marketing URLs (`APP_STORE_URL`, help docs, YouTube channel, and explainer video) are centralized in [`app/lib/marketing.ts`](app/lib/marketing.ts).

**iOS App Store marketing (UK):** Paste-ready listing copy, screenshot storyboard, and framed 1320×2868 assets live under [`marketing/appstore/uk/`](marketing/appstore/uk/). Start with [`COPY.md`](marketing/appstore/uk/COPY.md) and [`UPLOAD.md`](marketing/appstore/uk/UPLOAD.md). Auto-renewable subscription listings must keep the Terms of Use (EULA) footer in the App Description (Guideline 3.1.2); the UK COPY includes Apple’s standard EULA URL plus Ration Terms/Privacy.

**Why AI Gateway instead of calling Google AI directly?** The gateway provides request logging, cost analytics, caching, guardrails, spend limits, retries/timeouts, model fallback, and unified billing — configurable from the Cloudflare dashboard and via per-request `cf-aig-*` headers. All four AI consumers (`scan`, `meal_generate`, `plan_week`, `import_url`) route through a centralized client ([`app/lib/ai-gateway.server.ts`](app/lib/ai-gateway.server.ts)) that applies feature-specific timeouts, retries, caching policy, and observability metadata. Copilot uses the same gateway with **google-ai-studio BYOK** (`transport: "gateway"`); it does not use prepaid Unified Billing credits. The Google API key stays on the gateway; Workers authenticate with `CF_AIG_TOKEN` only.
...
**PDF receipts**: When a PDF is uploaded the consumer selects a receipt-specific prompt optimised for parsing grocery receipt line items (item name, weight/count columns, brand-name stripping). The metadata `source` field is set to `"pdf"` to distinguish PDF scans from image scans. Model output is parsed via `parseModelJson` (fence strip, balanced extract, trailing-comma / mid-array glitch repair, item-object reconstruction); on first parse failure the consumer retries once with `forceRefresh`. Parse/schema failures store customer-facing copy only — technical `JSON.parse` messages stay in server logs (with PII-light shape diagnostics).
...
- **Ingredient detail view** — `GET /hub/cargo/:id` shows full cargo metadata and all linked Galley meals. Links are **direct** (`cargoId`), **exact** (normalized name, including stripped prep words), **token** (e.g. `bread` ↔ `white bread`), or **vector** (semantic). Connected meal names link to Galley detail. Matching uses the same exact → token → vector resolver as cook/availability (`app/lib/ingredient-cargo-links.server.ts`); auto-matches are computed at read time and not written into `meal_ingredient.cargo_id`.
- **Cross-navigation** — Meal ingredient rows, supply list item names, hub expiring-cargo preview lines, and snoozed supply rows link to Cargo detail when an exact or token name match exists (`app/lib/cargo-links.ts`). Galley meal detail also resolves **vector** matches so a cookable ingredient is clickable. iOS mirrors this via `CargoLinkResolver` / `CargoTokenMatcher` and `resolvedCargoId` from the API. On iOS, opening a cargo detail always selects the Cargo tab (and meal detail selects Galley) so dock actions match the entity.
...
- **Quick Eat** (Flagship `cargo-quick-eat` + `nutrition-cook-log-split`) — `POST /api/cargo/:id/quick-eat` and `POST /api/mobile/v1/cargo/:id/quick-eat` ensure a linked provision, prepare today’s Manifest **snack**, best-effort pantry deduct (including when stock is 0), and optionally log **private intake for the acting user** (others still see Log serving / +). Clients preview scaled macros from the cargo nutrition snapshot and show the same planning-aid / not-medical-advice disclaimer as Manifest Eat; optional private notes (≤280) persist when `nutrition-intake-notes` is on. Count-unit pantry items keep household USDA servings when package mass is unknown, and meal recompute falls back to FDC household portion grams so Eat can attribute macros. When intake cannot be logged, web/iOS keep the sheet open with a skip reason (snack remains Prepared). iOS ships the yellow trailing **Eat** swipe + detail FAB (outline carrot); web Cargo card/list/detail expose **Eat** when `cargoQuickEat` is on (same API). Mark Empty remains detail-only.
- **USDA nutrition refresh** (Flagship `nutrition-engine`) — `POST /api/cargo/:id/nutrition/refresh` and `POST /api/mobile/v1/cargo/:id/nutrition/refresh` rematch the cargo row’s **server-side name** against USDA only (never AI), rate-limited (`cargo_nutrition_refresh`, 10/min fail-closed). Persists a USDA snapshot when matched; clears nutrition and returns a “none found” message when not. Web/iOS cargo detail show a refresh control next to the provenance chip (confirm before replacing a manual override). Manual create (web Add Item / iOS create) persists typed macros as `user_override` when the engine is on.
...
- **Vectorize write-through** — Every cargo create/update triggers `upsertCargoVector`, keeping the Vectorize index in sync with D1. Deletes call `deleteCargoVectors`, which chunks `deleteByIds` at 100 IDs (Workers payload cap; `VECTOR_DELETE_ERROR` 40007).
...
- **Week navigation** — The `?week=` query param shifts the visible window. **Calendar span** (`user.settings.manifestSettings.calendarSpan`: 3, 5, or 7 days) controls how many days appear on the Manifest page only (editable on Manifest, not Hub Settings). The Hub **manifest-preview** widget has its own per-user `daySpan` filter (Today / 3 / 7 / 14 days) via Customize Hub. When `nutrition-manifest` is on, tapping the center date range opens a **month calendar overlay** (green planned-meal dots; intake markers; dates older than ~13 months muted with “History kept for 13 months”). When both `nutrition-manifest` and `nutrition-goals` are on, day/week headers show the signed-in user’s preference-driven consumed/target strip (only set nutrients) or a **No goals** empty state linking to Settings. On **iOS**, calories stay a bolt + thin progress bar; protein / carbs / fat / fiber render as compact progress dials (warning color when over target). Web Manifest keeps the compact text strip. `GET /api/meal-plans/:id/planned-dates?from=&to=` returns `{ dates }` and optionally `{ consumedDates }`.
...
**Sign In vs Create Account:** These are separate intents. **Sign In** never creates a user row — Google and Apple use `disableImplicitSignUp`, and magic-link / social creation is blocked unless a short-lived ToS signup intent exists in `RATION_KV`. **Create Account** requires an unchecked clickwrap for Terms of Service + Privacy Policy; the client sends `requestSignUp` (social) or `metadata.requestSignUp` (magic link) with `tosAccepted: true`, and the server stores a signup intent before Better Auth may create the user. `databaseHooks.user.create.before` consumes that intent and stamps `tosAcceptedAt` / `tosVersion` only from verified consent (never fabricated on Sign In auto-provision). Unknown Sign In identities get `404 account_not_found` (“No account found. Create an account instead.”) **before** a magic-link email is sent. Mobile social Sign In uses the **same envelope**: if the ID token includes an email, the Worker checks D1 and returns that 404 **without calling Better Auth**; Hide-My-Email / missing-email Sign In still calls Better Auth and maps `signup_disabled` / `USER_NOT_FOUND` to the same 404 — never `401 authentication_failed` (App Store **1.3.x** drops the error `code` on 401, so the Create Account tab never appears). Apple `fullName` is forwarded to Better Auth only with `intent: signUp` (1.3.x always sends `fullName` on Sign In). On newer iOS, that code is also stored on `AuthManager` (process-level) so Sign in with Apple on iPad can recreate the login view without losing the **Create Account** tab + prompt. Sign In never retries D1 `server_busy`; only Create Account does.
...
**Native iOS (v1.4.49+):** The SwiftUI app supports **Sign in with Apple** and **Google Sign-In** via native SDKs. The app sends provider ID tokens to `POST /api/mobile/v1/auth/social`; the server verifies them through Better Auth and returns the same mobile JWT + refresh token pair as magic-link auth. Tokens are minted only after a readable org `member` row exists; `GET /session` and refresh **repair** a missing personal-org membership instead of 401 (which App Store **1.3.x** treats as forced logout). Concurrent refresh that loses the atomic claim returns `503 server_busy` or the KV grace pair — it does not revoke the refresh-token family. Required Worker secrets: `GOOGLE_IOS_CLIENT_ID` (GCP iOS OAuth client), `APPLE_APP_BUNDLE_IDENTIFIER` (`com.mayutic.ration`), plus existing `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` for web and ID-token audience verification. App Store Guideline 4.8 requires Sign in with Apple whenever Google social login is offered on iOS.

**App Review demo login:** For Guideline 2.1, Apple needs Username/Password in App Store Connect — not SSO alone. When Flagship `app-review-login` is enabled, typing `app-review@mayutic.com` on Sign In reveals a Password field; `POST /api/mobile/v1/auth/review-login` issues mobile JWTs for the pre-seeded account. Keep the demo user on **Free** tier (seed always forces Free) so reviewers see the full Crew subscribe paywall. See [`plans/app-review-notes.md`](plans/app-review-notes.md).
...
**Account deletion and ownership transfer:** Account delete is blocked while Crew is still set to auto-renew; after cancel-at-period-end (Stripe portal, App Store manage sheet, RevenueCat `CANCELLATION` webhook, or RevenueCat REST reconcile of `unsubscribe_detected_at`), users may delete with a warning that remaining paid access ends immediately. Stripe is cancelled **before** access is revoked. A KV purge-pending tombstone blocks re-login and mobile JWTs while hard purge runs in the background; durable purge jobs are retried by daily cron (after bounded session/queue_job deletes, not in parallel with unbounded D1 hygiene) and can also be retried from `/admin`. Failures email help@mayutic.com with flattened D1/Drizzle cause text. Before `DELETE FROM user`, wipe nulls leftover shared-kitchen user FKs (`cooked_by_user_id`, `reviewed_by_user_id`, `voided_by_user_id`, and similar) that SQLite treats as `ON DELETE NO ACTION`. Groups the user owns are handled as follows: if other members have joined, ownership auto-transfers to the first admin or first member. If the owner is the sole member (including when invitations are pending and not yet accepted), the group and all its data are permanently deleted. **Personal groups cannot be deleted standalone** (exact `personal-{userId}` slug or `metadata.isPersonal`) — the Danger Zone explains that account deletion is required to remove them. Owners can proactively transfer ownership to another member via the "Transfer ownership" option in group settings (Danger Zone) before deleting their account. Failed transfers show an inline error banner and toast with the specific reason (e.g. free recipients need Crew to own another group, or Crew recipients already at their owned-group limit). Members who cannot take ownership are disabled in the picker with an explanatory label.
...
**Admin dashboard (`/admin`):** God-mode metrics for `user.is_admin = true`. The page loader fetches critical counts, the first user page, and failed background purge jobs; heavy engagement metrics load progressively from `GET /api/admin/metrics` (KV-cached, rate-limited). Failed account/group wipes can be retried from the **Failed purges** panel (confirm by typing the account email or organization id; `admin_purge_retry`, fail-closed). The user table is paged on `user` then hydrated for the current page's session, mobile-token, and API-key activity — do not `GROUP BY` those tables in full. Grant admin with `bun run grant-admin -- user@example.com` (remote D1 `is_admin`). If `/admin` fails in production, tail worker logs (`bunx wrangler tail`) and smoke-test D1 query shapes with `bun run verify:admin-d1` (remote prod DB).
...
Published blog posts include category education such as [Pantry App vs Recipe Manager vs Meal Planner](/blog/pantry-app-vs-recipe-manager-vs-meal-planner), the connected-loop nutrition piece [Macro tracking that starts from dinner, not a second diary](/blog/macro-tracking-meal-planning-grocery), the sourced roundup [Best all-around kitchen apps](/blog/best-all-around-kitchen-apps), the iOS launch outline [Ration for iOS: One Kitchen, Every Surface](/blog/ration-ios-one-kitchen-every-surface), MCP onboarding guides such as [Your Kitchen Has an API Now](/blog/mcp-kitchen-assistant) (OAuth-first) and [Agent-First MCP Onboarding](/blog/agent-first-mcp-onboarding) (autonomous self-registration + claim-when-ready), plus Copilot coverage in [Agentic App Control Is the Next Interface](/blog/agentic-app-control-copilot) and [Building Ration Copilot on Cloudflare Think](/blog/building-ration-copilot-cloudflare-think). Blog posts with a `## FAQ` section emit `FAQPage` JSON-LD automatically for rich results and agent grounding.
...
| `adjust_cargo_item` | Write | `mcp:inventory:write` | Relative `delta` (e.g. -2); floors at 0; keeps the row. Pantry used/wasted only — personal eating uses `quick_eat_cargo` | mcp_write (15/min) |
...
**MCP resources & prompts:** The server also publishes static resources (`ration://units`, `ration://domains`, `ration://schemas/inventory-import`, `ration://capabilities`, connection guide) and prompts (`parse_receipt`, `plan_week`, `fit_remaining_macros`, `import_recipe_from_text`, `quick_eat_snack`) so agents can fetch canonical reference data and instruction templates without scraping documentation.

**No-credit default:** MCP tools are credit-free (rate-limited). Cargo writes set `skipVectorPhase: true` and backfill embeddings asynchronously so tool returns are not blocked. Billed Gemini jobs (Scan, URL import, Galley Generate, Plan Week) stay on native web/iOS only — not MCP or Copilot tools. Copilot fulfills “invent a meal / plan my week” with `create_meal` and `propose_manifest_plan` → `commit_manifest_plan`. Camera OCR scan and recipe URL extraction remain native deep-link flows (text receipt lists use preview/apply). The `get_credit_balance` tool was intentionally removed in favor of `get_billing_summary`.
...
**Authentication:** `Authorization: Bearer <access_jwt>`. Obtain tokens via a **PKCE-protected** magic link: the app generates a `code_verifier`, sends its S256 `codeChallenge` (`POST /api/mobile/v1/auth/magic-link { email, codeChallenge, intent, tosAccepted? }`) → email → continue page (POST verify) → `/auth/mobile-callback?client=ios&pending=...` → Universal Link handoff → `POST /api/mobile/v1/auth/token` with `grantType: authorization_code` **and `codeVerifier`** (server verifies `S256(verifier)`, rejecting a mismatch with `invalid_grant`). Refresh with `grantType: refresh_token`. PKCE binds the one-time code to the originating app so a hijacked `ration://` redirect cannot be redeemed. Magic-link **Sign In** rejects unknown emails with `404 account_not_found` before sending mail. Social auth (`POST /api/mobile/v1/auth/social`) uses the same `404 account_not_found` contract on Sign In (D1 email pre-check, then Better Auth `signup_disabled` / `USER_NOT_FOUND` mapped to 404 — never 401 for unknown users) so App Store **1.3.x** can switch to Create Account. Sign Up requires `tosAccepted: true` and may create an account; Sign In never auto-provisions. **`DELETE /api/mobile/v1/account`** uses user-only JWT auth (no active-org membership required) so account deletion works even when the access token references a stale group.
...
| `POST` | `/api/undo` | Reverse cook, manifest consume, or private intake within 5s (session; intake tokens are D1-backed) |
| `POST` | `/api/mobile/v1/undo` | Reverse cook, manifest consume, or private intake within 5s (intake tokens are D1-backed) |
...
| `POST` | `/api/mobile/v1/groups/delete` | Delete group (owner; `{ organizationId, acknowledgeCreditForfeit? }`). Positive remaining credits require `acknowledgeCreditForfeit: true` (credits are forfeited, not refunded); returns remaining `organizations` |
...
| `POST` | `/api/mobile/v1/cargo/:id/nutrition/refresh` | USDA-only nutrition rematch for cargo detail (persist or clear) |
...
bun run grant-admin -- user@example.com # Set user.is_admin on remote ration-db
...
**Nutrition flags** (F0 spine; registry default off): `nutrition-engine`, `nutrition-ai-estimate`, `nutrition-manifest`, `nutrition-goals`, `nutrition-cook-log-split`, `nutrition-async-recompute`, `nutrition-intake-notes` (`clientKey: nutritionIntakeNotes`). Bind `NUTRITION_DB` to `ration-nutrition` / `ration-nutrition-dev`. Local smoke seed: `bun run db:nutrition:seed:local` (`nutrition-db/schema.sql` + `nutrition-db/seed-minimal.sql`) — **never** promote the smoke seed to production/reviewers. Verified Foundation + SR Legacy (+ optional FNDDS) import: pin `nutrition-db/releases/current.json`, then `bun run db:nutrition:import:generate` (remote apply requires archive SHA pins; promote via binding change). After food rows exist, apply curated aliases with `bun run db:nutrition:alias:remote`. Matcher/KV keys are versioned by dataset snapshot + matcher (`1.4.0`); automated name matches are quality `high`, never `verified`. Main D1 stores cargo/meal snapshots plus `ingredient_nutrition_match`, `nutrition_consent`, `nutrition_goal`, `nutrition_intake` (optional private `notes` ≤280), request-level `nutrition_operation`, value-free `nutrition_access_audit`, and the `nutrition_recompute_job` outbox/lease ledger. Migrations `0045_*`–`0051_*` add release-hardening columns, indexes, ledgers, consent-withdrawal idempotency, stable committed operation results, durable intake-undo consumption, user-scoped operation keys, one open goal, deletion-safe recompute context, and intake notes; they are operator-applied before hardening code is enabled. Operator order: [nutrition-rollout.md](docs/dev/nutrition-rollout.md).

**Nutrition APIs:** `GET|POST /api/privacy/nutrition` and mobile `GET|POST /api/mobile/v1/privacy/nutrition` expose the user-global, versioned consent ledger and grant/withdraw/erase actions even while nutrition flags are off. Goal and Eat writes reject embedded consent booleans/timestamps and require active purpose consent; goals/intake rows retain the consent-row ID. `GET /api/nutrition/summary?from=&to=` reads private totals; `GET|POST|PATCH|DELETE /api/nutrition/goals` manages goals when enabled; mobile mirrors under `/api/mobile/v1/nutrition/*`. The canonical `NutritionService` owns scope, consent, flag, tenant, idempotency, conflict, audit, undo, and response semantics. Request-level UUID operation keys plus per-item keys make 1–50 item Eat/clear batches atomic and replayable; operation rows retain commit-time authoritative day totals and summaryGeneratedAt so later writes cannot change a replayed response. Intake undoTokens are the operationId and are consumed atomically in D1 (`undone_at`) within the same five-second window as cook/consume tokens; stale undo returns `409 undo_conflict`. `DELETE` goal/intake routes require `operationKey` in the query (or `Idempotency-Key`). When `nutrition-cook-log-split` is on, `POST /api/meal-plans/:id/entries/cook` is shared Cook and `POST|DELETE /api/meal-plans/:id/entries/:entryId/intake` is private Eat. Legacy Manifest Consume never creates personal intake, regardless of legacy `portions` or `logNutrition` payloads. MCP/Copilot nutrition access requires the narrow OAuth scope plus active underlying-purpose and `agent_processing` consent. Intake retention (~13 months) and completed/failed recompute-job retention run on the daily 03:00 D1 hygiene chain (bounded DELETEs). A 5-minute sweeper recovers missed nutrition recompute queue wakes (25 keys); queue `retry_delay` owns the hot path (`max_concurrency` 2). `POST /api/nutrition/resolve` and its mobile mirror propose nutrition snapshots when enabled.
...
- **Model:** hardcoded `google/gemini-3.7-flash` via **AI Gateway** `ration-gateway` (Think `LanguageModel` from [`model.server.ts`](app/lib/copilot/model.server.ts); the Copilot worker `AI` binding is transport only — there is no separate AI Gateway binding). Copilot forces `transport: "gateway"` so requests use **google-ai-studio BYOK** (the stored Google key, same as scan) — not the catalog `env.AI.run` path, which bills prepaid Unified Billing credits and fails with `Insufficient balance` when the gateway has none. The google plugin parses native `generateContent` (required for `thinkingConfig` / thoughts); openai stays registered as a run-path safety net. Kill switch is Flagship `ration-copilot` (404). Missing `AI_GATEWAY_ID` fails closed.
- **Presets** ([`model-profiles.ts`](app/lib/copilot/model-profiles.ts)):

| Preset | thinkingLevel | maxSteps | maxOutputTokens | stall | Use case |
|--------|---------------|----------|-----------------|-------|----------|
| **Fast** (default) | `low` | 8 | 8192 | 45s | Quick answers, lower token use |
| **Deep** | `high` | 16 | 16384 | 90s | Multi-step planning |

Fast/Deep use the same Gemini 3.7 Flash model. `providerOptions.google.thinkingConfig` sets `thinkingLevel` and `includeThoughts: true`. Think `sendReasoning: true` on **Ask and onboarding** streams `reasoning-*` frames into the Show thinking UI. Temperature / topP are omitted (Gemini 3+). Temporal context (today's UTC date) is appended each turn via [`formatCopilotTemporalContextAppend`](app/lib/agent/temporal-context.server.ts).

**System prompt:** [`getCopilotSystemPrompt`](app/lib/copilot/system-prompt.server.ts) — kitchen scope only, declines code/general knowledge, mandates `search_docs` for product questions, remaining-macros + Quick Eat workflows, URL/scan native redirects only, expired-is-expired expiry guidance, bulk import/remove rules (≥2 items), and post-mutation action reporting. Copilot kitchen tools are credit-free; the Ask conversation meter is the only Copilot charge.

### 14.5 Tools

Copilot exposes **shared MCP tools** through the shared tool runtime (including purpose-built `propose_manifest_plan` / `commit_manifest_plan` / `set_active_meals` / `mark_supply_purchased_bulk` / `preview_inventory_remove` / `apply_inventory_remove`, nutrition Cook/Eat tools `cook_manifest_entries` / `log_manifest_intake` / `clear_manifest_intake` / `list_nutrition_intakes` / `get_nutrition_summary` / `set_nutrition_goal` / `clear_nutrition_goal` / `quick_eat_cargo`), and Copilot-only `search_docs`. There are **no** billed Gemini tools on Copilot (`start_generate_meal` / `start_plan_week` were removed). Invent a meal with `create_meal`; fill a week with `propose_manifest_plan` → `commit_manifest_plan`. Each turn uses **intent-scoped `activeTools`** ([`active-tools.server.ts`](app/lib/copilot/active-tools.server.ts)) so only relevant write domains are enabled (`create_meal` and `propose_manifest_plan` stay in the core set; calorie/kcal/macro/nutrition/goal/ate/tonight/eat keywords unlock nutrition tools; multi-domain turns open nutrition hands too).

**Flagship:** Copilot product flags (nutrition, Quick Eat, native scan/import kill switches) inherit the Ask client's identity — iOS Ask evaluates as `ios` + marketing version from `X-Ration-Client`; web Ask evaluates as `web` + `APP_VERSION`. External MCP stays `mcp` + `APP_VERSION`. `agentSurface` remains `copilot` for kitchen-event source.
...
| `get_nutrition_summary` | Shared MCP | Personal UTC-day totals plus additive `vsGoal` remaining/overage (defaults to today) |
| `quick_eat_cargo` | Shared MCP | Personal Quick Eat — resolve/create cargo, Manifest snack, optional intake |
...
MCP scopes granted to Copilot: `mcp:read`, `mcp:inventory:write`, `mcp:galley:write`, `mcp:manifest:write`, `mcp:supply:write`, `mcp:preferences:write`, `mcp:nutrition:read`, `mcp:nutrition:write`. Tool calls inherit MCP rate categories (`mcp_list`, `mcp_search`, `mcp_write`, `mcp_supply_sync`) keyed on the synthetic credential `copilot:{userId}`.

**Not available as camera/URL tools in Copilot:** Receipt image scan and recipe URL import (including Instagram, TikTok, YouTube, and bare `https://` pastes) remain hard-blocked (use native Scan / Galley Import, or text → `preview_inventory_import` / MCP `create_meal` from extracted text). Credit-free scheduling uses `propose_manifest_plan` → `commit_manifest_plan`. Native Galley Generate / Manifest Plan Week stay on web/iOS HTTP only. See [§14.6 Intent guards](#146-intent-guards-and-native-feature-hints).
...
flowchart TD
    ...
    Blocked -->|scan / import_url| Block["Tools disabled — deep link to native Scan or Galley Import"]
    Blocked -->|no| Tools
```

- **Hard blocks** ([`intent-guard.server.ts`](app/lib/copilot/intent-guard.server.ts)): scan/OCR and recipe URL import (bare HTTPS, Instagram/TikTok/YouTube, social recipe phrasing) — emits `blocked_feature` with `ration://` deep links. These cannot continue in chat; `activeTools` is empty that turn.
- **No billed-AI upsell:** invent recipes with `create_meal`; plan a week with `propose_manifest_plan` → `commit_manifest_plan`. Scan and URL import remain the only native redirects.
...
| Session token cap | **256,000** tokens — billing hard stop, start new chat (max **13** credits/chat at 1/20k) |
| Session message cap | **240** messages |
