...
**Public splash experience:** `/` leads with the Ration crate mark + wordmark, the iOS App Store action, and a click-to-play YouTube explainer, then explains the closed kitchen loop: Cargo → Galley → Manifest → Supply → Dock. The loop diagram tracks the active stage while scrolling on desktop and mobile, with the logo anchoring the orbit core. The `#interfaces` section presents the three supported surfaces in priority order — iOS + Copilot, MCP, then Web + Copilot — using real product screenshots. A header light/dark control changes appearance for this page only (does not write the app theme cookie or hub settings). Subtle one-shot reveal motion is disabled by `prefers-reduced-motion`. `#pricing` advertises Crew’s 1 free Ask Ration conversation per group per day and shares the full Free vs Crew feature matrix with `/hub/pricing` via [`PricingFeatureMatrix`](app/components/pricing/PricingFeatureMatrix.tsx); `#pricing` and `#signup` deep links remain stable. Public marketing URLs (`APP_STORE_URL`, help docs, YouTube channel, and explainer video) are centralized in [`app/lib/marketing.ts`](app/lib/marketing.ts).
