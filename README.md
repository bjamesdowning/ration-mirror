...
- **UI** — Cargo/Galley panels, scan review (`POST /api/nutrition/resolve`), Manifest Eat plate-up + preference-driven day/week goal strip (only nutrients you set; **No goals** empty state) + ~13-month calendar, Settings goals (optional nutrients, consent). Personal kcal stays on user-scoped intake APIs — not in shared Flight Recorder event payloads.
