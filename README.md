...
- **Resolve** — USDA-shaped self-hosted `NUTRITION_DB` with FTS top-N + JS re-rank (KV `nutrition:fdc:v2:`); blank on miss for manual/CSV; labelled AI estimate only when request `ingestSource` is `scan_review` and `nutrition-ai-estimate` is on (client `allowAiEstimate` booleans are ignored). Mobile: `POST /api/mobile/v1/nutrition/resolve`. iOS scan review calls resolve after OCR, shows/edits macros on **Edit item**, and AI intros mention nutrition when `nutrition-engine` is on.
...
**Nutrition flags** (F0 spine; registry default off): `nutrition-engine`, `nutrition-ai-estimate`, `nutrition-manifest`, `nutrition-goals`, `nutrition-cook-log-split`, `nutrition-async-recompute`. Bind `NUTRITION_DB` to `ration-nutrition` / `ration-nutrition-dev`. Local seed: `bun run db:nutrition:seed:local` (`nutrition-db/schema.sql` + `nutrition-db/seed-minimal.sql`). Full FDC import (operator): unzip USDA CSV packs under `nutrition-db/raw/` (gitignored), then `bun run db:nutrition:import:generate` and `bun run db:nutrition:import:remote` (`scripts/import-fdc-nutrition.ts`; generated SQL also gitignored). Main D1 stores cargo/meal `nutrition` JSON snapshots plus `ingredient_nutrition_match`, `nutrition_goal`, and `nutrition_intake` (migrations `0041_*` … `0044_*` for Cook/Eat intake uniqueness); helpers in `app/lib/nutrition/persist.server.ts`. Operator order: [nutrition-rollout.md](docs/dev/nutrition-rollout.md).

**Nutrition APIs:** `GET /api/nutrition/summary?from=&to=` (goals or manifest flag; optional fiber on day/totals from intake `nutrientsJson`); `GET|POST|PATCH|DELETE /api/nutrition/goals` (goals flag; ≥1 nutrient; `consent: true` or legacy `consentAt`; macros nullable); mobile `GET /api/mobile/v1/nutrition/goals?asOf=YYYY-MM-DD` (optional client calendar day); `POST /api/nutrition/resolve` + mobile `POST /api/mobile/v1/nutrition/resolve` (engine flag; `ingestSource: "scan_review"` for labelled AI estimates); `GET /api/meal-plans/:id/planned-dates?from=&to=` and mobile `GET /api/mobile/v1/manifest/planned-dates?from=&to=` (month-scoped planned dots + optional `consumedDates` when nutrition-manifest; voided intakes excluded). When `nutrition-cook-log-split` is on: `POST /api/meal-plans/:id/entries/cook` (shared Cook) and `POST|DELETE /api/meal-plans/:id/entries/:entryId/intake` (private Eat; **explicit** first-use intake consent via `consent: true`). Mobile mirrors under `/api/mobile/v1/nutrition/*`, `/api/mobile/v1/manifest/cook`, and `/api/mobile/v1/manifest/entries/:entryId/intake`. MCP/Copilot parity: `get_nutrition_summary`, `set_nutrition_goal`, `clear_nutrition_goal`; cargo/meal tools surface `nutrition` when present; `consume_manifest_entries` accepts `portions` + `logNutrition`. Legacy Manifest consume accepts optional `portions` + `logNutrition` for plate-up intake logging when `nutrition-manifest` is on. Day/week views show preference-driven consumed/target strips (or **No goals**) when both nutrition flags are on. Intake retention (~13 months) runs with the kitchen-event cron purge. When `nutritionEngine` is on, Galley meal detail/edit, Cargo detail/edit, and scan review show `NutritionPanel`; scan review calls `POST /api/nutrition/resolve` to propose snapshots before batch add.
