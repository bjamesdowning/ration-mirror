...
- **Resolve** — USDA-shaped self-hosted `NUTRITION_DB` with curated `food_alias` short-circuit, FTS top-N + primary-prefix bank, JS re-rank (matcher `1.4.0`); optional live FDC search miss fallback when `USDA_FDC_API_KEY` is set. Blank on true miss/low for manual/CSV. Cargo photo scan and Supply dock **review** call `POST /api/nutrition/resolve` (mobile twin) in the background after review opens (chunked for progressive kcal): medium+ USDA attaches; labelled **AI Estimate** when `nutrition-ai-estimate` is on and `ingestSource` is `scan_review`. Review shows **Looking up nutrients…** plus per-row calorie placeholders until batches return. Snapshots carry into cargo ingest; confirm still AI-fills lines without a preview snapshot. Merge into a cargo row with null nutrition backfills USDA/AI from the incoming name.
