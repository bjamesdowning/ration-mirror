...
- **APIs / MCP** — Summary and goals routes (nullable macros; ≥1 nutrient on upsert; `consent: true` or legacy `consentAt`); tools `get_nutrition_summary`, `set_nutrition_goal`, `clear_nutrition_goal`. Intake retention ~396 days; erased on account purge.
- **Dogfood** — Production enablement via Flagship `userId` allowlist **+** `clientVersion` ≥ shipping iOS/web build (see [nutrition-rollout.md](docs/dev/nutrition-rollout.md)) — never `FEATURE_FLAG_OVERRIDES` to turn nutrition **on**. Keep registry defaults **off**; do not default flags on with “exclude old versions.”
