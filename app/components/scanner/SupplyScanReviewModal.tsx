...
export function SupplyScanReviewModal({
	...
	useEffect(() => {
		...
		// Capacity 403 — stay open; never treat as successful dock.
		if (fetcher.data.error === "capacity_exceeded") {
			setShowUpgradePrompt(true);
			return;
		}
		if (typeof fetcher.data.docked === "number" && fetcher.data.docked > 0) {
			onSuccess();
		}
	}, [fetcher.state, fetcher.data, onSuccess]);

	const hasCapacityError =
		fetcher.state === "idle" && fetcher.data?.error === "capacity_exceeded";
	const otherError =
		fetcher.state === "idle" &&
		fetcher.data?.error &&
			...
			<div className="fixed inset-0 z-50 flex items-end md:items-center justify-center modal-scrim p-0 md:p-4">
				<div className="modal-surface w-full md:max-w-2xl md:rounded-2xl shadow-xl max-h-[92vh] flex flex-col">
					...
					<div className="flex-1 overflow-y-auto p-4 space-y-3">
						{hasCapacityError && (
							<div className="p-4 rounded-lg bg-amber-500/15 border border-amber-500/40 flex items-start gap-3">
								...
								<div className="flex-1 min-w-0">
									...
									<p className="text-sm text-muted mt-1">
										Nothing was docked
										{typeof fetcher.data?.current === "number" &&
										typeof fetcher.data?.limit === "number" &&
										fetcher.data.limit > 0
											? ` — Cargo is at ${fetcher.data.current}/${fetcher.data.limit}`
											: ""}
										. Free plans allow a limited number of Cargo items. Deselect
										items to fit, or upgrade to Crew for unlimited capacity.
									</p>
					...
					{otherError && (
						<p className="px-4 pb-3 text-sm text-red-600">{otherError}</p>
					)}
				</div>
			</div>
			<UpgradePrompt
				...
				description={
					...
					typeof fetcher.data?.limit === "number" &&
					fetcher.data.limit > 0
						? `Cargo is at ${fetcher.data.current}/${fetcher.data.limit}. Upgrade to Crew Member for unlimited Cargo items, meals, and supply lists.`
						: "Upgrade to Crew Member for unlimited Cargo items, meals, and supply lists."
				}
			/>
		</>
	);
