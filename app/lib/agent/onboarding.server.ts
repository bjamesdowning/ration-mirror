...
export function buildNutritionCapabilityNotes(
	...
	if (flags.cookLogSplit) {
		notes.push(
			"nutrition-cook-log-split: Cook with cook_manifest_entries or consume_meal (Galley bridge) — never logs personal intake. Eat separately with log_manifest_intake after consent (servings 0.01–100, or amount+unit g/oz when gramsPerServing is present). Personal snacks: quick_eat_cargo. consume_manifest_entries is refused while split is on.",
		);
	}
	if (flags.manifest) {
		notes.push(
