...
URL imports follow a **completeness ladder**: full recipe → partial skeleton (ingredient names / steps without full quantities) → **link holder** (meal name from metadata + source URL saved). Holders are last-resort after captions, native transcripts, and spoken-audio transcription have been tried on social videos. You always leave with something to review for URL lanes—never an empty dead-end.
...
- **Social posts** — TikTok, Instagram, and YouTube (including Shorts). Ration pulls platform metadata (including Supadata title/description), then native captions, then **spoken-audio ASR** (`mode=generate`) when the caption is still too thin. A partial meal (ingredient names without amounts, or steps only) is a successful import; a link holder is used only after that ladder. On iOS, use **Share → Ration** from the social app — the share sheet URL and any caption text are forwarded. Status shows **Listening to the video…** while transcribing.
...
Import runs **asynchronously**. After you submit, the app shows **processing** (including **Listening to the video…** for social ASR) and **polls** until extraction finishes. You then **verify** the extracted recipe (badge: Full / Partial / Saved link, plus evidence chips such as “From spoken audio”) and confirm to add it to Galley. Meals keep `customFields.sourceUrl` so **View source** opens the original page or video.
