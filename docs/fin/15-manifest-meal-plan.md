...
- **Month calendar** — When Manifest nutrition is on, tap the center date range for a month overlay (planned-meal dots, intake markers; older than ~13 months muted). See *Macro tracking, goals, and intake*.
...
- **Cook / Prepared / Log my serving (flag-gated)** — When Cook/Log split is on, **Cook** deducts Cargo once and marks the entry *Prepared* (no personal nutrition). **Log my serving** / Eat opens a private plate-up for how much you ate: type any amount (0.01–100 servings) or use the stepper, and optional **g / oz** when the meal has recipe-ingredient mass (`gramsPerServing`). Mass is ingredient grams, not cooked plated weight; macros scale as `perServing × servings`. First personal log requires **Macro Tracking** in Feature enablement when that section is shown (not implied by Cook or goals). See *Macro tracking, goals, and intake*.
