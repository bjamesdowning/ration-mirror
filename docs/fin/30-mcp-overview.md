...
- **Recommended auth:** **OAuth 2.1** delegated access — paste the MCP URL into your client, complete browser sign-in, **always select your household**, then approve scoped permissions. Flow state is tracked server-side (KV, 600s TTL); see [plans/oauth-flow-contract.md](../../plans/oauth-flow-contract.md). Manage or revoke grants in **Hub → Settings → Connected Agents**.
