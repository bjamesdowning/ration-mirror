...
| **Authorize** does nothing; console `form-action 'self'` on `/oauth/consent` | CSP blocks redirect to `http://localhost:PORT/callback` (mcp-remote). Deploy >= v1.5.14 (routes via `/oauth/return`). Do not change CSP manually. |
