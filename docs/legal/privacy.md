...
When nutrition features are enabled for your account, you may optionally set **personal daily nutrition goals** (energy and macros) and log **private intake** when you choose to record a Manifest meal as eaten (plate-up). In the product UI this is offered as **Macro Tracking** under Feature enablement (onboarding and Settings). Enabling Macro Tracking records separate, versioned explicit consent for goals, intake, and connected-agent nutrition processing. The full statements remain available for review; nutrition write requests cannot grant consent implicitly or provide their own consent timestamp.

You may withdraw Macro Tracking (or any underlying nutrition purpose) in Settings → Feature enablement. Withdrawal blocks future processing but does not by itself erase previously stored data; goals and intake can be erased separately. Goals remain until you clear or erase them or delete your account. Intake history is retained for approximately **396 days (~13 months)** and then deleted automatically unless you erase it sooner. Deleting a shared household/kitchen does **not** by itself erase your personal intake history; those rows remain associated with your account (with remembered kitchen/meal labels where available) until retention or account purge. Nutrition goals and intake records are also erased when you purge your account (see Section 11).

### AI Features

**AI Features** (scan, generate, Ask Ration, and related cloud processing, including optional Copilot credit auto-deduct) are enabled through Feature enablement during onboarding or in Settings. Turning AI Features off clears processing consent and stops new AI use until you enable them again.
...
- **Supadata (Dumpling Software UG):** Recipe import helpers. When you import a website or social video URL, we send that URL to Supadata to fetch publicly available page content, post metadata, captions, and — when captions are missing — a speech-to-text transcript of the video audio. We do not upload the video file ourselves. Returned text may then be sent to Gemini (see above). See [Supadata's Privacy Policy](https://supadata.ai/privacy).
...
- **AI Providers (Google Gemini via Cloudflare AI Gateway):** We use Google Gemini (proxied via Cloudflare AI Gateway) to process the following categories of data:
  ...
  - **Recipe URLs, page content, and social video transcripts** — URLs you submit for recipe import, fetched page HTML, share-sheet captions, on-screen captions, and speech-to-text transcripts of public social videos (TikTok, Instagram, YouTube) when captions are missing. That text is sent to Gemini for structured recipe extraction. We do not download the video file into Ration.
