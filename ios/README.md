...
**Onboarding (iOS 1.1.43, web v1.6.1):** Ask-first welcome briefing with **starter kitchen seed**. New users see full-screen Ask Ration; Copilot auto-sends “What is Ration?”, then offers **Stock my kitchen** to add 5 staples (selective expiry + tags) via `add_cargo_item`. Server allowlists both prompts, forces Deep preset, enables tools only on the seed turn (`copilot-onboarding-free` flag). **Get Started** completes onboarding (opens Cargo when seeded). Settings → Tutorial replays static copy only via `restartOnboarding` PATCH.
