...
**Onboarding (iOS 1.1.46, web v1.6.4):** Ask-first welcome briefing with **starter kitchen seed**. Live path (flag on): Deep intro streams fully → ~0.9s settle → inline **Suggested** “Stock my kitchen” chat bubble (not a bottom modal) → tap when ready to seed. **Get Started** stays available after intro (skip mid-seed); composer Stop works while a turn is active. Fallback path (flag off / ineligible): static welcome + **See in Cargo** / **Get Started** chips — never a dead-end. Settings → Tutorial replays static copy only via `restartOnboarding` PATCH.
