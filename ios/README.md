...
Settings PATCH accepts `hubProfile` and `hubLayout` for customizable Hub widgets. Per-widget filters include meal tags, manifest day span (1/3/7/14), supply cargo tags, slot/domain, and item limits — synced with web `hubLayout`. In Hub edit mode (toolbar slider), reorder widgets with the system List grip (`EditMode` + `onMove`); tap a widget row to open **Configure** (one density control + filters). List widgets use **Show up to N** (2–12) as the sole density control (`filters.limit`); Manifest uses **Days to show**; Quick Stats uses Compact/Standard/Expanded. Size (`sm`/`md`/`lg`) is auto-synced for web grid parity and no longer caps item count on iOS. Layout changes autosave. Copilot dock is hidden while Hub edit is open (other tabs keep Copilot). Exit with **Done** or by re-tapping the Hub tab to return to the rendered custom hub. Hub item taps push meal/cargo detail on the Hub `NavigationStack` (not a sheet) so Copilot FAB actions stay reachable. **Appearance** (Settings → Light/Dark segmented control) updates `user.settings.theme` and syncs with the web app; choice is cached in UserDefaults for instant cold start.
...
**Post-buildout polish (v1.4.4–1.4.5):** Conditional sync indicator in toolbar (offline/stale only); structured recipe directions with step UI; unified AI intro+form flows; hub widgets are tappable; manifest preview supports day-span filters; Supply uses thin progress bar + icon dock FAB. **v1.4.5** adds icon-only action menus on Cargo/Galley/Manifest/Hub, hub layout presets (Full/Cook/Shop/Minimal), and slot glyphs on manifest rows.
...
**Copilot immersive chat (iOS 1.1.7 build 8):** Dock and full-screen chat now share a floating composer that starts as one line, grows with soft wrap up to ten lines via native `TextField` intrinsic height, submits from the send arrow (Return inserts newlines in the Ask sheet), and dismisses interactively with a downward swipe. The full chat uses a compact single-row header, full-width assistant responses, right-aligned user bubbles, stable distance-based auto-follow, and a morphing dock-to-chip transition. Multi-turn streaming now appends each response after its prompt; session-limit recovery and late-frame filtering match web behavior.

**Copilot reliability pass (iOS 1.1.12 build 8):** Restores Copilot send/stream lifecycle parity with web — non-response agent frames are ignored, event observation is ready before connect, and the composer clears after send while the keyboard stays open. Sticky activity shows during connect/thinking until assistant tokens arrive. Scroll tracking refreshes when tabs activate so dock collapse works on Cargo/Galley/Manifest/Supply. Composer height remeasures after UIKit layout instead of at width ≈ 1pt.

**Onboarding (iOS 1.2.37, web v1.6.40):** Ask-first welcome briefing with a **single-item milk seed** (expiry + dairy tag). New human accounts also get a starter **Hot Chocolate** Galley recipe at signup. Live path (flag on): Fast intro (`search_docs`) → settle → Suggested bubble with seed prompt → tap to run seed. **60s** turn timeout + retry (same conversation for grant); soft-success if `add_cargo_item` already succeeded when the watchdog fires; empty/failed intro surfaces retry; late frames don’t wipe timeout errors. Get Started force-cancels hung turns. Fallback path (flag off / ineligible): static welcome + **See in Cargo** / **Get Started** chips — never a dead-end. Settings → Tutorial replays static copy only via `restartOnboarding` PATCH.
...
**Detail dock FAB (iOS 1.1.22 / 1.4.23):** Cargo item and Galley meal detail screens register their `⋯` action menus in `CopilotBottomDock` via `.tabDockAction` (same chrome/animations as list `+` FABs). `TabDockContext` uses per-tab **root** and **detail** layers so a detail `⋯` cannot LIFO-pop the list `+` when TabView hides an inactive tab. Cross-tab deep links (Manifest, connected meals, ingredient taps) wait until the destination tab is selected before pushing the detail path. Detail scroll surfaces use `.copilotDockScrollMargins` instead of legacy `safeAreaInset` FABs.

**Red delete / yellow soft swipe (iOS 1.2.17):** Cargo and Galley share trailing soft+hard swipes via `ListSwipeActions` — soft action is warning yellow (`Theme.warning`) and is the full-swipe default; Delete is danger red and never full-swipes. Cargo soft = **Mark Empty** (`0.circle`, sets qty to 0); Galley soft = **Cook** (`flame`). Manifest/Supply keep Delete-only trailing swipes with `allowsFullSwipe: false`. Cargo edit forms allow quantity 0; create still requires > 0.

**Dietary restriction indicators (iOS 1.2.32):** Settings → Dietary restrictions saves `user.settings.allergens` and updates `LaunchCoordinator` live. Galley meal rows show a compact warning glyph when ingredient names match selected allergens (same keyword map as web `app/lib/allergens.ts`); meal detail shows a full “Contains allergens” banner with labels. Matching is client-side via `AllergenDetector` — no extra API fields.
