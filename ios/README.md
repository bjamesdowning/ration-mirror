...
**Onboarding (iOS 1.1.45, web v1.6.3):** Ask-first welcome briefing with **starter kitchen seed**. Live path (flag on): Deep intro → **Stock my kitchen**. Fallback path (flag off / ineligible): static welcome + **See in Cargo** / **Get Started** chips — never a dead-end. Settings → Tutorial replays static copy only via `restartOnboarding` PATCH.
