...
**Onboarding (iOS 1.1.48, web v1.6.6):** Ask-first welcome briefing with **starter kitchen seed**. Live path (flag on): Fast intro (`search_docs`) → settle → Suggested bubble with full seed prompt visible → tap to run seed. 45s turn timeout + retry (same conversation for grant); empty/failed intro surfaces retry; late frames don’t wipe timeout errors. Get Started force-cancels hung turns. Fallback path (flag off / ineligible): static welcome + **See in Cargo** / **Get Started** chips — never a dead-end. Settings → Tutorial replays static copy only via `restartOnboarding` PATCH.
