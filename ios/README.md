...
**Onboarding (iOS 1.1.47, web v1.6.5):** Ask-first welcome briefing with **starter kitchen seed**. Live path (flag on): Fast intro (`search_docs`) → settle → Suggested bubble with full seed prompt visible → tap to run seed. 45s turn timeout + retry; Get Started cancels an in-flight turn. Fallback path (flag off / ineligible): static welcome + **See in Cargo** / **Get Started** chips — never a dead-end. Settings → Tutorial replays static copy only via `restartOnboarding` PATCH.
