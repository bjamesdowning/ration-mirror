...
final class AIConsentCoordinatorTests: XCTestCase {
    ...
    /// skips the prompt.
    @MainActor
    func testConsentGrantedByOneEntryPointIsVisibleToAnotherWithoutASecondPrompt() {
        let session = SessionStore()
        ...
        var generateRan = false
        generateCoordinator.presentIfNeeded(session: session) { generateRan = true }

        XCTAssertTrue(generateRan)
        XCTAssertFalse(generateCoordinator.isPresenting)
    }
}
