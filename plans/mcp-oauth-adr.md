...
- **Browser flow:** Better Auth drives login → post-login (select household) → consent → code. Optional KV `oauth:flow:{id}` is telemetry only (see [oauth-flow-contract.md](./oauth-flow-contract.md)).
